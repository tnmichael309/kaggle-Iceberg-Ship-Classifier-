...
__all__ = ['sesnet', 'IceSEBasicBlock', 'senetXX_generic']
...
class IceResNet(nn.Module):
    def __init__(self, block, n_size=1, num_classes=1, num_rgb=2, base=32, n_meta_features=1):
        ...
        #self.layer4 = self._make_layer(block, self.inplane * 4, blocks=2 * n_size, stride=2)
        self.avgpool = nn.AdaptiveAvgPool2d(1)

        self.fc = nn.Linear(int(8 * self.base)+n_meta_features, num_classes)
...
def senetXX_generic(num_classes=1, num_rgb=2, base=32, meta_features=1):
    model = IceResNet(IceSEBasicBlock, 1, num_classes, num_rgb, base, meta_features)  # 56
    return model
