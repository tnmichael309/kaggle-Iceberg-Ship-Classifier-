...
from data_set_api import ContrastDataset, PLIcebergDataset
...
class ContrastTrainer:
    ...
    def get_data_loader(self, df1, df2, is_train, is_transfer_learning, params={}):
    ...
    def train(self, train_df, valid_df, test_df=None, test_ids=None, is_transfer_learning = False, data_augmentation_args={},
        ...
        if pl_enabled is False:
            train_data_loader = self.get_data_loader(train_df, train_df, True, is_transfer_learning, data_augmentation_args)
        else:
            train_data_loader = self.get_pl_data_loader(train_df, train_df, True, is_transfer_learning, data_augmentation_args)
        
        
        if valid_df is not None:
            valid_data_loader = self.get_data_loader(valid_df, train_df, False, is_transfer_learning, data_augmentation_args)
    ...
    def train_epoch(self, data_loader, is_show=False):
        ...
        for img1, img2, labels in data_loader:
            ...
            img1 = Variable(self.to_gpu(img1))
            img2 = Variable(self.to_gpu(img2))
            labels = Variable(self.to_gpu(labels))
    ...
    def valid_epoch(self, data_loader, is_ensemble=False, test_df=None):
        ...
        for img1, img2, labels in data_loader:
            img1 = Variable(self.to_gpu(img1), volatile=True)
            img2 = Variable(self.to_gpu(img2), volatile=True)
    ...
    def test(self, train_df, test_df, is_transfer_learning = False, is_general_model=False, is_augment=False, data_augmentation_args={}):
        # set the model status (train\test)
        self.model.train(False)

        final_predictions = []
        final_top_3_avg_predictions = []
        
        for i in range(test_df.shape[0]):
            
            fake_test_df = pd.concat([test_df.iloc[i]]*train_df.shape[0], ignore_index=True)
            ...
            predictions = []
            for img1, img2, _ in test_data_loader:
                img1 = Variable(self.to_gpu(img1), volatile=True)
                img2 = Variable(self.to_gpu(img2), volatile=True)
                outputs = self.model(img1, img2)
            ...
            predictions = np.concatenate(predictions, axis=0)
            sorted_pred_ix = predictions.argsort()
            top_3_similar = sorted_pred_ix[-3:][::-1]
            top_3_unsimilar = sorted_pred_ix[:3]
            
            similar_ans = []
            unsimilar_ans = []
            
            for j in range(3):
                s_id = top_3_similar[j]
                
                train_ans = train_df.iloc[s_id, 'is_iceberg']
                if train_ans == 0.0:
                    similar_ans.append(1.0 - predictions[s_id]) #ex: predictions[s_id] = 0.8
                else:
                    similar_ans.append(predictions[s_id])
                    
                us_id = top_3_unsimilar[j]
                train_ans = train_df.iloc[us_id, 'is_iceberg']
                if train_ans == 0.0:
                    unsimilar_ans.append(1.0 - predictions[us_id]) #ex: predictions[s_id] = 0.2
                else:
                    unsimilar_ans.append(predictions[us_id])
            
            final_predictions.append((similar_ans[0]+unsimilar_ans[0]) / 2.0)
            final_top_3_avg_predictions.append(np.concatenate([similar_ans, unsimilar_ans], axis=0).mean())
            
        return final_predictions, final_top_3_avg_predictions
