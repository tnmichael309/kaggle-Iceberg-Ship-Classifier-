...
def dpn92(num_classes=1000, meta_features = 1):
    return DPN(num_init_features=64, k_R=96, G=32, k_sec=(3,4,20,3), inc_sec=(16,32,24,128), num_classes=num_classes, n_meta_features=meta_features)
...
class DualPathBlock(nn.Module):
    def __init__(self, in_chs, num_1x1_a, num_3x3_b, num_1x1_c, inc, G, _type='normal', p=0.0):
...
class DPN(nn.Module):
    ...
    def __init__(self, num_init_features=64, k_R=96, G=32,
                 k_sec=(3, 4, 20, 3), inc_sec=(16,32,24,128), num_classes=1000, n_meta_features=1):
        ...
        inc = inc_sec[0]
        R = int((k_R*bw)/256)
        blocks['conv2_1'] = DualPathBlock(num_init_features, R, R, bw, inc, G, 'proj', p=.0)
        in_chs = bw + 3 * inc
        for i in range(2, k_sec[0]+1):
            blocks['conv2_{}'.format(i)] = DualPathBlock(in_chs, R, R, bw, inc, G, 'normal', p=.0)
            in_chs += inc
        ...
        inc = inc_sec[1]
        R = int((k_R*bw)/256)
        blocks['conv3_1'] = DualPathBlock(in_chs, R, R, bw, inc, G, 'down', p=.0)
        in_chs = bw + 3 * inc
        for i in range(2, k_sec[1]+1):
            blocks['conv3_{}'.format(i)] = DualPathBlock(in_chs, R, R, bw, inc, G, 'normal', p=.0)
            in_chs += inc
        ...
        inc = inc_sec[2]
        R = int((k_R*bw)/256)
        blocks['conv4_1'] = DualPathBlock(in_chs, R, R, bw, inc, G, 'down', p=.0)
        in_chs = bw + 3 * inc
        for i in range(2, k_sec[2]+1):
            blocks['conv4_{}'.format(i)] = DualPathBlock(in_chs, R, R, bw, inc, G, 'normal', p=.0)
            in_chs += inc
        ...
        inc = inc_sec[3]
        R = int((k_R*bw)/256)
        blocks['conv5_1'] = DualPathBlock(in_chs, R, R, bw, inc, G, 'down', p=.0)
        in_chs = bw + 3 * inc
        for i in range(2, k_sec[3]+1):
            blocks['conv5_{}'.format(i)] = DualPathBlock(in_chs, R, R, bw, inc, G, 'normal', p=.0)
            in_chs += inc
        ...
        if n_meta_features > 0:
            ...
            self.bn = nn.BatchNorm1d(n_meta_features , momentum=0.1)
        self.n_meta_features = n_meta_features
        
        self.classifier = nn.Linear(in_chs + n_meta_features, num_classes)
        self.prob = nn.Sigmoid()
        

    def forward(self, imgs, other_features=None):
        
        features = torch.cat(self.features(imgs), dim=1)
        out = F.avg_pool2d(features, kernel_size=5, stride=3, ceil_mode=True).view(features.size(0), -1)
        #out = self.classifier(out)
        #return out
        #print(out)
        
        if self.n_meta_features > 0:
            other_features = self.bn(other_features)
